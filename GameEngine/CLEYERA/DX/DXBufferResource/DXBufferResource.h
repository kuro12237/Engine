...
#pragma endregion
  ...
  void DFCreateBuffer(size_t size, D3D12_RESOURCE_FLAGS flags,
...
template <typename T>
inline void DXBufferResource<T>::Init(size_t instanceNum) {
...
template <typename T> inline void DXBufferResource<T>::Map() {
  buffer_->Map(0, nullptr, reinterpret_cast<void **>(&this->param_ptr_));
}

template <typename T> inline void DXBufferResource<T>::Map(void **p) {
  buffer_->Map(0, nullptr, p);
}

template <typename T> inline void DXBufferResource<T>::UnMap() {}

template <typename T>
inline void DXBufferResource<T>::GraphicsRootDescripterTable(UINT num) {
  auto list = Base::DX::DXCommandManager::GetInstace();
  D3D12_GPU_DESCRIPTOR_HANDLE handle =
      descripterManager_->GetSRVGPUHandle(srvHandleIndex_);
  list->GraphicsDescripterTable(num, handle);
}

template <typename T>
inline void DXBufferResource<T>::ComputeRootDescripterTable(UINT num) {
...
template <typename T>
inline void DXBufferResource<T>::SetParam(std::vector<T> param) {
...
template <typename T>
inline void DXBufferResource<T>::CreateVertexBufferView() {
...
template <typename T>
inline void DXBufferResource<T>::CreateBuffer(D3D12_HEAP_PROPERTIES heapParam,
...
template <typename T>
inline void
DXBufferResource<T>::DFCreateBuffer(size_t size, D3D12_RESOURCE_FLAGS flags,
                                    D3D12_RESOURCE_STATES initialState,
                                    D3D12_HEAP_TYPE heapType) {
...
template <typename T>
inline void DXBufferResource<T>::WriteMemory(const void *pData,
                                             size_t dataSize) {
